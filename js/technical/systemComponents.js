	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : '需要' + formatWhole(tmp[layer].requires) + tmp[layer].baseResource + '解锁(你有' + formatWhole(tmp[layer].baseAmount) + tmp[layer].baseResource + ')')
			)
			: (
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                ...
                <td><button class="opt" onclick="toggleOpt('offlineProd')">离线时间: {{ player.offlineProd?"开":"关" }}</button></td>
            </tr>
            <tr>
            ...
            <tr>
                <td><button class="opt" onclick="toggleOpt('hideChallenges')">通关的挑战显示状态: {{ player.hideChallenges?"隐藏":"显示" }}</button></td>
